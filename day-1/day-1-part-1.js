...
const inputText1 = `1abc2
pqr3stu8vwx
a1b2c3d4e5f
treb7uchet`;

const inputText2 = `ckmb52fldxkseven3fkjgcbzmnr7
...
function calibrateDocument(input) {
  const textLines = input.split('\n');

  let totalCalibrationValue = 0;

  for (let line of textLines) {
    totalCalibrationValue += findCalibrationValue(line);
  }

  return totalCalibrationValue;
}

function findCalibrationValue(lineInput) {
  let leftPointer = 0;
  let rightPointer = lineInput.length - 1;

  while (isNaN(lineInput[leftPointer])) {
    leftPointer++;
  }

  while (isNaN(lineInput[rightPointer])) {
    rightPointer--;
  }

  let stringValue = lineInput[leftPointer] + lineInput[rightPointer];

  return Number(stringValue);
}

console.log(calibrateDocument(inputText1)); // answer = 142
console.log(calibrateDocument(inputText2)); // answer = 53386
